...
class TestIntegrations(TestCase):
    ...
    def test_bad_group(self):
        ...
        self.assertEqual(response.status_code, 404)
        response = self.client.get("/groups/query?gid=1234")
        self.assertEqual(response.status_code, 404)
    ...
    def test_single_user_queries(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"), self.users_to_json("root:x:0:0:root:/root:/bin/bash")
        )
        response = self.client.get("/users/query?name=gopher")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"),
            self.users_to_json("gopher:x:13:30:gopher:/var/gopher:/sbin/nologin"),
        )
        response = self.client.get("/users/query?uid=14")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"),
            self.users_to_json("ftp:x:14:50:FTP User:/var/ftp:/sbin/nologin"),
        )
        response = self.client.get("/users/query?uid=99")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"),
            self.users_to_json("nobody:x:99:99:Nobody:/:/sbin/nologin"),
        )
    ...
    def test_multiple_user_queries(self):
        ...
        self.assertEqual(
            response.data.decode("utf-8"),
            self.users_to_json("gdm:x:42:42::/var/gdm:/sbin/nologin"),
        )
        response = self.client.get(
            "/users/query?name=bin&uid=1&gid=1&home=/bin&comment=bin&shell=/sbin/nologin"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"), self.users_to_json("bin:x:1:1:bin:/bin:/sbin/nologin")
        )
    ...
    def test_groups_by_user(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"), self.groups_to_json("cdrom:x:24:username, username1")
        )
    ...
    def test_single_groups_queries(self):
        ...
        response = self.client.get("/groups/query?gid=27")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.decode("utf-8"), self.groups_to_json("sudo:x:27:"))
        response = self.client.get("/groups/query?gid=22")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.decode("utf-8"), self.groups_to_json("voice:x:22:"))
        response = self.client.get("/groups/query?gid=42")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data.decode("utf-8"), self.groups_to_json("shadow:x:42:"))

    def test_multiple_groups_queries(self):
        response = self.client.get(
            "/groups/query?name=cdrom&gid=24&member=username&member=username1"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"), self.groups_to_json("cdrom:x:24:username, username1")
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data.decode("utf-8"), self.groups_to_json("plugdev:x:46:username")
        )
